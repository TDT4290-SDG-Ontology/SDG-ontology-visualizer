...
def insert_data(token, kpi, value, municipality, year, dataseries=None):
    if dataseries:
        req = requests.post(BASE_URL + "/data/insert", json={'token': token["token"], 'indicator': kpi, 'data': value,
                                                             'municipality': municipality, 'year': year,
                                                             'isDummy': True, 'dataseries': dataseries})
    else:
        req = requests.post(BASE_URL + "/data/insert", json={'token': token["token"], 'indicator': kpi, 'data': value,
                                                             'municipality': municipality, 'year': year,
                                                             'isDummy': True})
...
def insert_bulk_data(token, municipality, year, data):
    req = requests.post(BASE_URL + "/data/insert-bulk",
                        json={'token': token["token"], 'municipality': municipality, 'year': year, 'data': data,
                              'isDummy': True})
    print(req.status_code, req.reason)
...
def set_goal(token, municipality, kpi, target, deadline, baseline, baselineYear, start_range, dataseries):
    if dataseries:
        req = requests.post(BASE_URL + "/gdc/set-goal",
                            json={'token': token["token"], 'indicator': kpi, 'municipality': municipality,
                                  'target': target, 'deadline': deadline, 'baseline': baseline,
                                  'baselineYear': baselineYear, 'startRange': start_range, 'dataseries': dataseries,
                                  'isDummy': True})
    else:
        req = requests.post(BASE_URL + "/gdc/set-goal",
                            json={'token': token["token"], 'indicator': kpi, 'municipality': municipality,
                                  'target': target, 'deadline': deadline, 'baseline': baseline,
                                  'baselineYear': baselineYear, 'startRange': start_range, 'isDummy': True})
    print(req.status_code, req.reason)


def set_bulk_goals(token, municipality, goals):
    req = requests.post(BASE_URL + "/gdc/set-bulk-goals",
                        json={'token': token["token"], 'municipality': municipality, 'goals': goals, 'isDummy': True})
    print(req.status_code, req.reason)
...
def generate_data(token, municipality, goal_goodness, data_goodness, year):
    if SEND_BULK_DATA:
        ...
        for ds in u4ssc.all_dataseries:
            insert_data(token, ds.kpi, ds.produce_data(goal_goodness, data_goodness, year), municipality, year,
                        ds.variant)
...
def test_data_generation():
    def test(goal_goodness, data_goodness, start_range, end_range):
        ...
        print("penalty_pct: {:.2f}, rate_penalty: {:.2f}, interest: {:.4f}, base: {:.4f}, randomness: {:.2f}\n".format(
            penalty_pct, rate_penalty, interest, base, randomness))
